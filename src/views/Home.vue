<template>
  <v-form>
    <v-container>
      <v-row>
        <v-col cols="12" sm="6">
          <v-text-field v-model="ime" label="Upisi ime" filled></v-text-field>
          <v-btn color="success" class="mb-5" @click="getData"
            >Prikazi podatke</v-btn
          >
        </v-col>
      </v-row>
      <div>
...
export default {
  ...
  methods: {
    async getData() {
      let rezultat = await fetch("https://api.nationalize.io?name=" + this.ime);
      let podaci = await rezultat.json();
      this.detaljime = podaci;

      let rezultat2 = await fetch("https://api.agify.io?name=" + this.ime);
      let podaci2 = await rezultat2.json();
      this.detaljime2 = podaci2;

      let rezultat3 = await fetch("https://api.genderize.io?name=" + this.ime);
